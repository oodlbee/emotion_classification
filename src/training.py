...
def lda_classification(X_featured, df_target, write_folder):
    ...
    # SVM classification
    clf = SVC()
    par = {'C': [1, 10, 100, 1000], "gamma": [1, 0.1, 0.001, 0.0001],
              "degree": range(1, 15),"kernel": ["linear", "rbf"]}
    ...
    with open(write_folder.joinpath("scaler.pkl"), "wb") as file:
        pickle.dump(scaler, file)


    with open(write_folder.joinpath("pca.pkl"), "wb") as file:
        pickle.dump(pca, file)

    with open(write_folder.joinpath("lda_model.pkl"), "wb") as file:
        pickle.dump(LDA_model, file)

    with open(write_folder.joinpath("svd_model.pkl"), "wb") as file:
